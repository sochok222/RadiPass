    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->actionNew->setIcon(iconCreateNew);
    ...
    ui->actionConfigure_Columns->setIcon(iconSetting);
    // Connecting buttons on user toolbar
    connect(ui->newDatabaseButton, SIGNAL(clicked()), SLOT(createDatabase()));
    connect(ui->openDatabaseButton, SIGNAL(clicked()), SLOT(openDatabase()));
    // Connecting actions
...
void MainWindow::customMenuRequested(QPoint pos){
    ...
    if(ui->tableView->indexAt(pos).isValid() && ui->tableView->underMouse()) // If mouse cursor is on tableView and on row
    {
        ...
        actionCopyUsername.reset(new QAction(iconUser, "Copy User Name", this)); // Resetting QAction *actionCopyUsername object
        actionCopyPassword.reset(new QAction(iconKey, "Copy Password", this)); // Resetting QAction *actionCopyPass object
        actionEdit.reset(new QAction(iconEdit, "Edit", this)); // Resetting QAction *actionEdit object
        actionAdd.reset(new QAction(iconAdd, "Add new", this)); // Resetting QAction *actionAdd object
        actionDelete.reset(new QAction(iconTrash, "Delete", this)); // Resetting QAction *actionDelete object
        actionCfgColumns.reset(new QAction(iconSetting, "Configure colums", this));

        // Setuping list of action to url
        subMenuUrl.reset(new QMenu("URL", this));
        subMenuUrl->setIcon(iconUrl);
        actionCopyUrl.reset(new QAction(iconCopy, "Copy", this));
        actionOpenUrl.reset(new QAction(iconOpenUrl, "Open", this));
